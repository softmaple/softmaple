---
title: 'Development'
description: 'Run and verify the Softmaple stack'
---
...
WebSocket service (the default collaboration runtime), an optional Cloudflare
Durable Objects runtime, Prisma/Supabase migrations, the Lexical editor, a
TanStack Start playground, and the EG-walker collaboration packages.

```text
Browser → /collab/* → apps/collab → Redis (realtime) + Supabase Postgres (history)
       ↘ apps/web → Supabase Auth / Data API / Storage

Optional: apps/web can route selected documents to apps/collab-cloudflare
(Durable Objects + the same Postgres history). See
[collaboration operations](/design/collaboration-operations).
```
