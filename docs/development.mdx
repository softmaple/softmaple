...
handshakes through that gateway (not against `next dev` directly). Production
on Vercel uses the `app/collab/*` WebSocket bridge instead of an external
rewrite. Preview validation of the real handshake remains a required release
gate.
