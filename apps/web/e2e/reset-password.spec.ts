...
test.describe("Password Reset Flow", () => {
  ...
  test("should display the update password page", async ({ page }) => {
    ...
    // Check link back to login
    await expect(page.locator('a[href="/login"]')).toContainText(
      "Back to sign in",
    );
  });
  ...
  test("should navigate to reset password form from login page", async ({
    page,
  }) => {
  ...
  test.skip("should complete full password reset flow with email verification", async ({
    page,
  }) => {
