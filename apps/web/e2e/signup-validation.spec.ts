...
test.describe("Signup Form Validation", () => {
  ...
  test("should display all required fields including names", async ({ page }) => {
    ...
    // Check placeholders
    await expect(page.getByLabel("First name")).toHaveAttribute("placeholder", "John");
  ...
  await expect(page.locator("div").filter({ hasText: "Passwords do not match" }).first()).toBeVisible();
...
test("should clear error when user starts typing", async ({ page }) => {
  ...
  await expect(page.locator("div").filter({ hasText: "Passwords do not match" }).first()).not.toBeVisible();
});
...
test("should preserve form data when validation fails", async ({ page }) => {
  ...
  test("should validate required fields with HTML5 validation", async ({ page }) => {
    // Try to submit with empty required fields
    await page.locator('button[type="submit"]').click();

    // Check HTML5 validation on first name field
    const firstNameValidity = await page.getByLabel("First name").evaluate(
