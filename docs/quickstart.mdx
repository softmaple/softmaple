...
- pnpm 11
- A Supabase project (PostgreSQL credentials for that project)
- Git
...
<AccordionGroup>
  ...
  <Accordion icon="rocket" title="6. Start the development servers">
    ...
    ```bash Terminal 1 — collaboration service
    pnpm --filter @softmaple/collab dev
    ```

    ```bash Terminal 2 — Next.js (internal)
    pnpm --filter @softmaple/web exec next dev --port 3001
    ```

    ```bash Terminal 3 — same-origin /collab router
    ...
    `apps/collab/.env.local`.

    Alternatively, use `vercel dev` from the repository root so the Services
    routing in `vercel.json` serves `/collab/*` from `apps/collab`.
...
<CardGroup>
  ...
  >
    Runtime layers, consistency, and operations.
  </Card>
