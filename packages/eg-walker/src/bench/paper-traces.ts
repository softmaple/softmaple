...
/**
 * Convert paper transaction patches into SoftMaple GraphEvents.
 *
 * This is intentionally patch-level conversion. Long inserted strings remain a
 * single insert operation, because this benchmark is designed to stress raw
 * trace ingest and replay at package scale before adding stricter keystroke
 * level modes.
 */
  ...
  for (let txnIndex = 0; txnIndex < trace.txns.length; txnIndex++) {
    ...
    for (let patchIndex = 0; patchIndex < txn.patches.length; patchIndex++) {
      ...
      if (granularity === "operation") {
        for (let deleteIndex = 0; deleteIndex < deleteLength; deleteIndex++) {
          ...
          const id = txn._dtSpan
            ? `paper:${dataset}:lv:${lv}`
            : `paper:${dataset}:txn:${txnIndex}:patch:${patchIndex}:del:${deleteIndex}`;
          events.push({
            id,
        ...
        let insertOffset = 0;
        for (const char of Array.from(insertedText)) {
          const insertIndex = index + insertOffset;
          const utf16Index = utf16IndexForCodePointIndex(
            currentUnicodeState,
            insertIndex,
          );
          const id = txn._dtSpan
            ? `paper:${dataset}:lv:${lv}`
            : `paper:${dataset}:txn:${txnIndex}:patch:${patchIndex}:ins:${insertOffset}`;
          events.push({
            id,
            parentVersion: cloneVersion(currentVersion),
            operation: {
    ...
    if (granularity === "operation" && txn._dtSpan !== undefined) {
      const expectedEnd = txn._dtSpan[1];
      if (lv !== expectedEnd) {
        throw new Error(
          `${dataset}: txn ${txnIndex} emitted operation ids through ${lv}, expected ${expectedEnd}`,
        );
      }
    }
