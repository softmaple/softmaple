...
Playwright and production both use `apps/web/scripts/collab-gateway.mjs` for
signed document and Presence handshakes (not bare `next dev` / `next start`).
Validate the real gateway handshake before promotion.
