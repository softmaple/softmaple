...
import type { GraphEvent, EventId } from "../types";
...
describe("EventGraph", () => {
  ...
  describe("getTopologicalOrder", () => {
    ...
    it("uses buildLinearHistory to verify topological ordering on deep chains", () => {
      const { graph, ids } = buildLinearHistory(5);
      const ordered = graph.getTopologicalOrder();
      expect(ordered.map((e) => e.id)).toEqual(ids);
    });
  });
