...
import type { Room, Document, User, SyncMessage } from './types';
...
export class RoomManager {
  ...
  private initSyncAdapter(_roomId: string, user: User): void {
    ...
    this.syncAdapter.on('remote-event', async (msg: SyncMessage) => {
      if (msg.userId !== user.id && this.api) {
        try {
  ...
  async handleLocalChange(operation: 'insert' | 'delete', position: number, text?: string, length?: number): Promise<void> {
    if (!this.api || !this.currentRoom) return;
 ...
 delete(position: number, length: number): void {
   this.handleLocalChange('delete', position, undefined, length).catch(console.error);
 }
