...
<AccordionGroup>
  ...
  <Accordion icon="rocket" title="6. Start the development servers">
    ...
    <Note>
      ...
      Local Playwright uses `scripts/e2e-collab-gateway.mjs`; Vercel Preview
      uses the App Router + `@vercel/functions` bridge under `app/collab/`.
      Do not restore a direct unsigned browser URL as a workaround.
    </Note>
  </Accordion>
</AccordionGroup>
