...
| `NEXT_PUBLIC_COLLAB_CLOUDFLARE_WS_URL` | Deprecated alias for the row above, read whenever `COLLAB_CLOUDFLARE_WS_URL` is blank or unset, so existing deployments keep working. Being a `NEXT_PUBLIC_*` value, changing it still requires a rebuild. |
